const { CellNode, XvCell, TagCell, RichTagCell } = require("./shared/cellNode");
const {
  findCellTag,
  reTag,
  reBlockSplit,
  isVariableNoSpace,
  isXv,
} = require("./shared/cellTag");
const { Cell } = require("./exceljs.lib");
const RichTextExceljs = require("./richText");
...
class CellModel {
  ...
  get isNullCell() {
    return (
      this.model.type === Cell.Types.Null ||
...
function getCellNode(model, row, col) {
  ...
  const cellModel = new CellModel(model);
